        ...
        var tem=this
           ...
           return(
               ...
               <View style={styles.contain}>
                   <TouchableOpacity style={styles.sure} onPress={()=>{
                       this.state.isChangeName==true
                       postFetch(API.BuChongOrder,{expressageExcepRecord:{orderId:list,reason:this.state.item,refuseReason:this.state.changeTime}},
                           (result)=>{
                               // alert(JSON.stringify(result))
                               // alert(this.state.item)
                               if(result.status==1){
                                   setTimeout(function () {
                                       tem._toast.show('提交成功')
