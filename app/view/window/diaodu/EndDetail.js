...
export default class EndDetail extends Component{
    constructor(props){
        super(props)
        this.state={
            time:0,//订单时间
        ...
        postFetch(API.DingDan,{expressageOrder:{id:list}},(result)=>{
            ...
            if(result.status==1){
                this.setState({
                    ...
                    checkStatus:result.data.checkStatus
                })
                ...
                <ScrollView style={{width:Contants.Screen.width}} >
                    <TouchableOpacity style={styles.top} onPress={()=>{this.props.navigation.navigate('ZhiPaiWho',{data:list})}}>
                        ...
                        <View  style={{alignItems:'center',justifyContent:'center'}}>
                            <Text style={{fontSize:14,color:this.state.checkStatus==1?"#33BAB2":'#FF305E'}}>{this.state.checkStatus==0?'待审核':this.state.checkStatus==1?'已同意':'已拒绝'}</Text>
                        </View>
