...
import ChatView from './ChatIosView'
...
export default class ChatViews extends Component{
    ...
    componentDidMount() {
        postFetch(API.TotalHuifu,null,(result)=>{
            ...
            this._MapView.initChatView(list,0,JSON.stringify(result).toString())
        })
      ...
      // this.subscription=  DeviceEventEmitter.addListener('event',this.onResult)
        ...
        this.listener=DeviceEventEmitter.addListener('userDetail',this.onresult)
    }
    onresult=(e)=>{
        if(e.action){
                if(this.state.tags===e.action){
                   this.props.navigation.navigate('MessageMain')
                }else {
                   this.props.navigation.navigate('UserDetails',{hxid:e.action})
                }
        }
    ...
    componentWillUnmount(){
        ...
        // NativeModules.ChatModule.removeChatViewNotification()
    ...
    render(){
        return(<View style={{flex:1,backgroundColor:'#f9f9f9'}}>
            ...
            <View  style={styles.headerContainer}>
                ...
                <Text allowFontScaling={false} style={styles.middleTitle}>消  息</Text>
