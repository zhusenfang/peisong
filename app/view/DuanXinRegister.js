...
export default class DuanXinRegister extends Component {
    mixins: [TimerMixin]
    constructor(props){
        super(props);
        this.state={
        ...
        return(<View style={styles.contain}>
            ...
            <View style={{flexDirection:'row'}}>
                <CommonTextInput
                    ...
                    maxLength={6}
                    onEnd={t=>this.setState({
                        content:t
                    })}
                    inputItemStyle={{marginTop:30,marginLeft:10}}
                    // onChange={}
                    // inputItemStyle={{color:'red',fontSize:14}}
                />
            </View>
            ...
            <TouchableOpacity style={[styles.zuixia,{marginTop:50,}]} onPress={this.sourAction}>
                <Image source={require('../img/shezhi/chongzhi.png')} style={styles.zuixia}>
                    <Text style={styles.fa}>确定登录</Text>
                </Image>
            </TouchableOpacity>
            <Toast
                ref={(e) => {
    ...
    sourAction=()=>{
        ...
        postFetch(API.DuanXinLogin,{phone:list,code:code},(result)=>{
            // alert(JSON.stringify(result))
            if(result.status==1){
                toast.show(result.msg)
                Storage.save("isLogin",true);
                Storage.save("phoneNumber",list);
                Storage.save("userId",result.data.id)
